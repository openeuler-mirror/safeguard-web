...
class SensorService:
    ...
    def install_sensor(config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if exit_code != 0:
                raise OperationError(f"获取系统版本失败: {stderr}")
            ...
            if '"7"' in os_release and "centos" in os_release:
                ...
                raise OperationError(f"不支持的系统类型: {stdout}")

            # 2. 获取架构
            stdout, stderr, exit_code = ssh.execute_command("uname -m")
            if exit_code != 0:
                raise OperationError(f"获取架构失败: {stderr}")
            ...
            if arch == "x86_64":
                ...
                raise OperationError(f"不支持的架构: {arch}")
            ...
            if exit_code != 0 or not stdout.strip():
                raise OperationError(f"未找到 RPM 包: {rpm_pattern} in {base_path}")
            ...
            if exit_code != 0:
                raise OperationError(f"安装失败: {stderr}")

            return {"job_id": job_id, "message": f"Sensor 安装成功: {rpm_file}"}
    ...
    def update_config(serial_number: str, override_config: Dict[str, Any],
        ...
        try:
            ...
            if exit_code != 0:
                raise OperationError(f"读取配置失败: {stderr}")

            try:
                base_config = toml.loads(stdout)
            except toml.TomlDecodeError as e:
                raise OperationError(f"解析 TOML 失败: {e}")
            ...
            if exit_code != 0:
                raise OperationError(f"写入配置失败: {stderr}")

            # 4. 重启 sensor 服务
            stdout, stderr, exit_code = ssh.execute_command("systemctl restart sensor")
            if exit_code != 0:
                raise OperationError(f"重启服务失败: {stderr}")

            return {"config": merged, "message": "配置更新并重启成功"}
    ...
    def operate_sensor(config: Dict[str, Any], operate: str) -> Dict[str, Any]:
        ...
        if operate not in valid:
            raise OperationError(f"无效操作: {operate}, 支持 {valid}")
        ...
        try:
            ...
            if exit_code != 0:
                raise OperationError(f"操作失败: {stderr}")

            return {"output": output, "message": f"sensor {operate} 成功"}
