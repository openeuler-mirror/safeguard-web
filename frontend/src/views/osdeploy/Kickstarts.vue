  ...
  <div class="kickstarts-container">
    <div class="kickstarts-header">
      <div class="header-actions">
        ...
        />
        <button class="btn-primary" @click="openAutoGenerateDialog">自动全量生成</button>
...
export default {
  ...
  methods: {
    ...
    openGenerateDialog(ks) {
      this.selectedKickstart = ks
      this.generateVarsJson = '{}'
    ...
    async doGenerate() {
      try {
        const variables = JSON.parse(this.generateVarsJson || '{}')
        const res = await generateKickstart(this.selectedKickstart.id, variables)
        this.generateContent = res.content || ''
      } catch (e) {
    ...
    openAutoGenerateDialog() {
      this.autoGenHostId = ''
      this.autoGenRepoId = ''
      this.autoGenOsType = 'culinux'
      this.generateContent = ''
      this.loadHostsForAutoGen()
      this.autoGenerateDialogVisible = true
    },
    closeAutoGenerateDialog() {
      this.autoGenerateDialogVisible = false
      this.generateContent = ''
    },
    async loadHostsForAutoGen() {
      try {
    ...
    async doAutoGenerate() {
      if (!this.autoGenHostId || !this.autoGenRepoId) {
        alert('请选择主机和仓库')
        return
      }
      try {
        const res = await autoGenerateKickstart({
          host_id: this.autoGenHostId,
          repo_id: this.autoGenRepoId,
          os_type: this.autoGenOsType,
        })
        this.generateContent = res.content || ''
      } catch (e) {
        alert(e.message || '自动生成失败')
      }
    },
    formatDate(dateStr) {
      if (!dateStr) return '-'
